...
Coral_path <- "C:/Users/matt.dornback/R_Working_Directory/DeepSeaCorals/DSCRTP_NatDB_20190418-0_Science_Fields/DSCRTP_NatDB_20190418-0_Science_Fields_Arctic.csv"
...
EMU <- EMU %>% filter(OceanName == "Arctic") %>% select(OBJECTID, POINT_X, POINT_Y) %>% rename(Longitude = POINT_X, Latitude = POINT_Y)
...
#
Coral$nearest_EMU <- apply(spDistsN1(Coral, EMU,longlat = TRUE), 1, which.min)
...
write.csv(FinalTable,file="FinalEMUAssignments2.csv")
